...
void testRandomPlaceHolder() {
    for (uint32_t size = 1; size < 6; size++) {
        ...
        KFKVInfo("holder 0x%x for size %u", holder, size);
    }
}
...
void AESCrypt::testAESCrypt(const void *key, size_t keyLength, const uint8_t plainText[], size_t textLength) {
    ...
    KFKVInfo("AES CFB decode: %s", decryptText);
...
void AESCrypt::testAESCrypt() {
    ...
    {
        const uint8_t plainText[] = "Hello, OpenSSL-kfkv::AESCrypt::testAESCrypt() with AES CFB 128.";
    ...
    {
        const uint8_t plainText256[] = "Hello, OpenSSL-kfkv::AESCrypt::testAESCrypt() with AES CFB 256.";

...
#    endif
    ...
    if (unLockFirstIfNeeded) {
        ...
        if (ret != 0) {
            KFKVError("fail to try unlock first fd=%d, ret=%d, error:%s", m_fd, ret, strerror(errno));
        }
    }
    ...
    if (ret != 0) {
        ...
        if (wait) {
            KFKVError("fail to lock fd=%d, ret=%d, error:%s", m_fd, ret, strerror(errno));
        }
        ...
        if (unLockFirstIfNeeded) {
            ...
            if (ret != 0) {
                ...
                KFKVError("fail to recover shared-lock fd=%d, ret=%d, error:%s", m_fd, ret, strerror(errno));
            }
        }
        return false;
    } else {
...
#    endif
    ...
    if (flock(m_fd, cmd) != 0) {
        KFKVError("fail to unlock fd=%d, error:%d(%s)", m_fd, errno, strerror(errno));
        return false;
    }
    return true;
}
...
void FileLock::destroyAndUnLock() {
    ...
    m_sharedLockCount = 0;
    m_exclusiveLockCount = 0;
    m_fd = KFKVFileHandleInvalidValue;
}

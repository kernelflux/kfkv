...
KFKVBuffer CodedInputData::readRealData(kfkv::KFKVBuffer & data) {
    CodedInputData input(data.getPtr(), data.length());
    return input.readData(false, true);
}

KFKVBuffer CodedInputData::readData(bool copy, bool exactly) {
    int32_t size = this->readRawVarint32();
    if (size < 0) {
        throw length_error("InvalidProtocolBuffer negativeSize");
    }
    ...
    if (isSizeValid) {
        ...
        m_position += s_size;
        auto copyFlag = copy ? KFKVBufferCopy : KFKVBufferNoCopy;
        return KFKVBuffer(((int8_t *) m_ptr) + pos, s_size, copyFlag);
    } else {
        throw out_of_range("InvalidProtocolBuffer truncatedMessage");
    }
}
